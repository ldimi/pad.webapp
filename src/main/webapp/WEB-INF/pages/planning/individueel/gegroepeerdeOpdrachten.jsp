...
<div style="height: 30px; padding-top: 5px;">
                ...
                <tr>
                    ...
                    <td>
                        <select name="doss_hdr_id"
                            onchange="window.open('/pad/s/planning/individueel/gegroepeerdeOpdrachten?doss_hdr_id='+this.options[this.selectedIndex].value,'_top')"
                            class="input">
                            <option value=""></option>
                    ...
                    <c:if test="${fn:length(gegroepeerdeOpdrachtenDD) > 0}">
                        <td>A-dossier:</td>
                        <td><select name="dossier_id" class="input">
                                <c:forEach var="item" items="${gegroepeerdeOpdrachtenDD}">
    ...
    <c:if test="${gegroepeerdeOpdrachtenDD.size() == 0}">
        <div style="margin: 5px;">Geen gegroepeerde opdrachten te plannen.</div>
    </c:if>
...
<div id="bestekDetailsDialog" class="hidden">
    <div id="bestekDetails_grid_div" style="position: absolute; top: 2px; left: 2px; right: 2px; bottom: 28px;"
        class="slick-grid-div"></div>
    <button name="selectBtn" class="hidden"  style="position: absolute; height: 20px; width: 80px; left: 5px; bottom: 2px;"  >selecteer</button>
</div>
<div id="faseDetailsDialog" class="hidden">
    <div id="faseDetails_grid_div" style="position: absolute; top: 2px; left: 2px; right: 2px; bottom: 28px;"
        class="slick-grid-div"></div>
    <button name="selectBtn" class="hidden" style="position: absolute; height: 20px; width: 80px; left: 5px; bottom: 2px;" >selecteer</button>
</div>
