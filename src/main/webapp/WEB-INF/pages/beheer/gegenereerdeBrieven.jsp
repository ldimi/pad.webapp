...
<h1>Te scannen uitgaande brieven </h1>
...
<table>
    ...
    <tbody>
    <c:forEach items="${brievenZonder}" var="brief" varStatus="status">
        <c:choose>
        ...
        <tr class="${styleclass}">
            ...
            <td><input type="button" onclick="scanOpladen('/pad/s/beheer/gegenereerdeBrieven/opladen/${brief.id}')" value="Scan opladen"/></td>
        </tr>
    </c:forEach>
    </tbody>
</table>
<h1>Gescande uitgaande brieven </h1>
<table>
    <thead>
    <tr>
        <th>Uit Datum</th>
    ...
    <tbody>
    <c:forEach items="${brievenMet}" var="brief" varStatus="status">
        <c:choose>
            <c:when test="${status.index mod 2 == 0 }">
                <c:set var="styleclass" value="tableRowEven"/>
            </c:when>
        ...
        <tr class="${styleclass}">
            ...
            <td>${brief.laatsteWijzigingUser}</td>
            <td>
                <a href='<%=System.getProperty("ovam.dms.webdrive.base") %>/${brief.scanDmsFolder}/${brief.scanDmsFileName}'
