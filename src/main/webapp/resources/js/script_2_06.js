function popupWindow(file,window) {
    msgWindow=open(file,window,'resizable=yes,width=1150,height=600,scrollbars=yes,left=0,top=0');
...
function setCrudAction(form, action) {						
	form.crudAction.value = action;
	return true;			
}
function setActionAndName(form, action, name) {						
	form.action = action;
	form.name = name;			
	return true;			
}
function setActionArtikelId(form, action, artikelid) {						
	form.action = action;
	form.artikelid.value = artikelid;					
	return true;			
}	
function setConfirmAction(form, action, message) {		
...
function setSearchParameters(form, forwardURL, searchFlag, searchForward) {
	form.forwardURL.value = forwardURL;
	form.searchFlag.value = searchFlag;
	form.searchForward.value = searchForward;
	return true;
}
...
function sum(cellId, tableId, columnnr) {
	var sumArray = new Array(cellId.length);
	for ( var j = 0; j < sumArray.length; j++ ) { 
		var total = 0;
		var v_array = document.getElementsByName(cellId[j]);
		for ( var i = 0; i < v_array.length; i++ ) { 				
			var waarde = v_array[i].innerHTML - 0;         
			total = total + waarde;							  		       
		}
		sumArray[j] = total;				
	}					
	addRowToTable(tableId,columnnr, sumArray, "");	
}			

function sumdossier(cellId, tableId, columnnr) {
	var sumArray = new Array(cellId.length);	
	var dossierArray = null;
	var dossier = "";	
	var total = 0;
	var waarde = 0;	
	var v_array = null;
	for ( var j = 0; j < sumArray.length; j++ ) { 
		total = 0;	
		dossierArray = new Array();			
		v_array = document.getElementsByName(cellId[j]);
		for ( var i = 0; i < v_array.length; i++ ) { 
			// check if dossier has already occured;
			dossier = v_array[i].innerHTML.substring(0, v_array[i].innerHTML.indexOf("d"));
			if (inArray(dossierArray, dossier)) {
				
			}
			else {
				dossierArray[dossier] = dossier;
				waarde = v_array[i].innerHTML.substring(v_array[i].innerHTML.indexOf("d") + 1, v_array[i].innerHTML.length)-0;         
				total = total + waarde;	
			}								
	...
	addRowToTable(tableId,columnnr, sumArray, "Dossier");
}	



function sumkadaster(cellId, tableId, columnnr) {
	var sumArray = new Array(cellId.length);	
	var kadasterArray = null;
	var kadaster = "";	
	var total = 0;
	var waarde = 0;	
	var v_array = null;
	for ( var j = 0; j < sumArray.length; j++ ) { 
		total = 0;	
		kadasterArray = new Array();			
		v_array = document.getElementsByName(cellId[j]);
		for ( var i = 0; i < v_array.length; i++ ) { 
			// check if dossier has already occured;
			kadaster = v_array[i].innerHTML.substring(0, v_array[i].innerHTML.indexOf("k"));
			if (inArray(kadasterArray, kadaster)) {
				
			}
			else {
				kadasterArray[kadaster] = kadaster;
				waarde = v_array[i].innerHTML.substring(v_array[i].innerHTML.indexOf("k") + 1, v_array[i].innerHTML.length)-0;         
				total = total + waarde;	
			}								
	...
	addRowToTable(tableId,columnnr, sumArray, "Kadaster");
}			
...
function addRowToTable(id, columnnr, sumArray, title)
{
	...
	var currentCol = 0;			
	for ( var j = 0; j < sumArray.length; j++ ) { 								
		for (var columns=currentCol; columns < columnnr[j]; columns++) {
			var cell = row.insertCell(columns);						
		}
		currentCol = columnnr[j]+1;	 						
		var cell = row.insertCell(columnnr[j]);
		cell.className = "total";						
		var textNode = document.createTextNode(formatNumber(sumArray[j]));			
...
//******************************************************************************
    ...
    var scwBaseYear        = scwDateNow.getFullYear()-30;
    ...
    document.writeln(
        '<style type="text/css">'                                       +
            ...
            '/* Calendar background */'                                 +
            'table.scw                  {background-color:  #FBB117;}'  +
            '/* Drag Handle */'                                         +
            ...
            'td.scwCellsExMonth         {background-color:  #CCCCCC;'   +
                                        'color:             #666666;}'  +
            '/* Today selector */'                                      +
            'td.scwFoot                 {background-color:  #E8A317;'   +
                                        'color:             #FFFFFF;}'  +
