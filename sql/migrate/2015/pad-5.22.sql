...
alter table art46.bestek
   foreign key fk_be_screening_jn(screening_jn)
           references art46.ja_nee_code (code) on delete restrict
;



drop view ART46.BESTEK_GEEN_RECENTE_FACTUUR;create view ART46.BESTEK_GEEN_RECENTE_FACTUUR as with bestek_recente_factuur as ( 	select 		ab.* 	from 		ART46.BESTEK ab, 		ART46.BESTEK_SAP_PROJECT absp, 		SAP.PROJECT_FACTUUR spf 	where 		ab.afsluit_d is null and 		ab.bestek_id = absp.bestek_id and 		absp.project_id = spf.project_id and 		days (current date) - days (spf.betaal_d) < 180 ) select 	distinct ab.* from 	ART46.BESTEK ab, 	ART46.BESTEK_SAP_PROJECT absp, 	SAP.PROJECT sp where 	ab.afsluit_d is null and 	ab.bestek_id = absp.bestek_id and 	absp.project_id = sp.project_id and 	coalesce(sp.check_factuur_mail_d, '1971-01-01') < current date and 	ab.bestek_id not in (select bestek_id from bestek_recente_factuur);

-- screening_jn toevoegen aan v_bestek;
...
insert into art46.db_versie(db_versie) values ('5.22');
