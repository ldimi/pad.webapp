...
drop TRIGGER ART46.BU_DOSSIER_DOSSIER_NR;

alter table ART46.DOSSIER
    drop constraint CC_DOSSIER_B;

ALTER TABLE ART46.DOSSIER
      DROP CONSTRAINT DOSSIER_NR_UNIEK;


alter table art46.dossier
    rename column dossier_nr to dossier_id
;
...
CREATE TRIGGER ART46.BU_DOSSIER_DOSSIER_ID BEFORE
UPDATE ON ART46.DOSSIER REFERENCING NEW AS N OLD AS O FOR EACH ROW WHEN
(
   O.DOSSIER_ID not like '\_%' ESCAPE '\'
   and N.DOSSIER_ID <> O.DOSSIER_ID
)
...
values
(
   '_' || rtrim(cast(nrow.dossier_id as char(10))), nrow.dossier_id, 'B', 13
)
;
...
DROP VIEW SMEG.ART46_DOSSIER_VIEW

CREATE VIEW SMEG.ART46_DOSSIER_VIEW
(
   ID,
   DOSSIER_ID,
   DOSSIER_ID_BOA,
...
d.id,
d.dossier_id,
CAST(d.dossier_id_boa AS BIGINT) AS dossier_id_boa,
d.dossier_b,
coalesce(d.programma_code, '') programma_type_b,
AMBTENAAR_B AMBTENAAR_NAAM,
TEL1 AMBTENAAR_TEL
FROM ART46.DOSSIER d left outer
join ART46.OVAM_AMBTENAAR_VIEW oa on (oa.ambtenaar_id = d.doss_hdr_id)
;


delete from  art46.db_versie
where db_versie = '6.13';
...
alter table art46.dossier
    rename column dossier_id to dossier_nr
;
...
ALTER TABLE ART46.DOSSIER
      ADD CONSTRAINT DOSSIER_NR_UNIEK UNIQUE(DOSSIER_NR)
;


alter table ART46.DOSSIER
    add constraint CC_DOSSIER_B check (
                                        (DOSSIER_NR not like '\_%' ESCAPE '\' and DOSSIER_B is not null and DOSSIER_B <> '')
                                        OR
                                        (DOSSIER_NR like '\_%' ESCAPE '\' and DOSSIER_B is null and DOSSIER_TYPE = 'B' )
                                        OR
                                        (DOSSIER_NR like '\_a\_%' ESCAPE '\' and DOSSIER_B is null and DOSSIER_TYPE = 'A' )
                                     );     

CREATE TRIGGER ART46.BU_DOSSIER_DOSSIER_NR BEFORE
UPDATE ON ART46.DOSSIER REFERENCING NEW AS N OLD AS O FOR EACH ROW WHEN
(
   O.DOSSIER_NR not like '\_%' ESCAPE '\'
   and N.DOSSIER_NR <> O.DOSSIER_NR
)
SIGNAL SQLSTATE '75000' SET MESSAGE_TEXT='Dossier_nr mag niet gewijzigd worden'
;

-- herdefinieren van v_dossier
-- invalid views
-- update nakijken.
-- queries nakijken. (ok)
...
insert into art46.dossier
(
   dossier_nr, dossier_id_boa, dossier_type, dossier_fase_id
)
values
(
   '_' || rtrim(cast(nrow.dossier_id as char(10))), nrow.dossier_id, 'B', 13
...
from syscat.VIEWS v
where v.viewschema = 'SMEG'
     and v.valid = 'N'
;


-- deze versie van de wijzigingen in db registreren.
insert into art46.db_versie(db_versie) values ('6.13');
