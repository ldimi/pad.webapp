...
alter table art46.dossier
    add foreign key FK_DOS_BELEID_RISI (beleidsmatig_risico_id)
         references art46.screening_prioriteit_waarde (waarde_id)
         on delete restrict
;

alter table art46.dossier
    add foreign key FK_DOS_INTEGR_RISI (integratie_risico_id)
         references art46.screening_prioriteit_waarde (waarde_id)
         on delete restrict
;

alter table art46.dossier
    add foreign key FK_DOS_ACTUEEL_RISI (actueel_risico_id)
         references art46.screening_prioriteit_waarde (waarde_id)
         on delete restrict
;

alter table art46.dossier
    add foreign key FK_DOS_POTENTIEEL_RISI (potentieel_risico_id)
         references art46.screening_prioriteit_waarde (waarde_id)
         on delete restrict
;
...
set (
        ...
        where dossier_id = dos.id
              and deleted_jn = 'N'
       )
where dos.id in (select dossier_id from art46.dossier_overdracht)
; 
